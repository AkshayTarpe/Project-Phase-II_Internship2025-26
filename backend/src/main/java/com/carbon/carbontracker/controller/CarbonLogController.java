...
import com.carbon.carbontracker.dto.HeroEmissionPoint;
...
public class CarbonLogController {
    ...
    @GetMapping("/public/hero-emissions")
    public ResponseEntity<List<HeroEmissionPoint>> getPublicHeroEmissions() {
        LocalDate today = LocalDate.now();
        LocalDate weekStart = today.minusDays(6);

        List<CarbonLog> logs = carbonLogRepository.findByDateBetween(weekStart, today);
        ...
        for (CarbonLog log : logs) {
            ...
            BigDecimal current = totals.getOrDefault(logDate, BigDecimal.ZERO);
            BigDecimal logTotal = log.getTotalEmission() != null ? log.getTotalEmission() : BigDecimal.ZERO;
            totals.put(logDate, current.add(logTotal));
        }

        String[] labels = {"Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"};
        List<HeroEmissionPoint> response = new java.util.ArrayList<>();

        for (int index = 0; index < labels.length; index++) {
            LocalDate currentDate = weekStart.plusDays(index);
            BigDecimal total = totals.getOrDefault(currentDate, BigDecimal.ZERO);
            response.add(new HeroEmissionPoint(labels[index], total.doubleValue()));
        }
