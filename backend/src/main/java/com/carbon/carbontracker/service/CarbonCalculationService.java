...
public class CarbonCalculationService {
    ...
    public double calculateTransport(SurveyRequest request) {
        ...
        final double bikeMultiplier = settingsStoreService.getDouble("transportBikeMultiplier", 0.2);
        ...
        switch (request.getTransportMode().toUpperCase()) {
            ...
            case "CAR":
                ...
                if ("PETROL".equalsIgnoreCase(fuel)) {
                    factor = settingsStoreService.getDouble("transportCarPetrol", 0.21);
                } else if ("DIESEL".equalsIgnoreCase(fuel)) {
                    factor = settingsStoreService.getDouble("transportCarDiesel", 0.18);
                } else if ("ELECTRIC".equalsIgnoreCase(fuel)) {
                    factor = settingsStoreService.getDouble("transportCarElectric", 0.05);
                } else {
                    factor = settingsStoreService.getDouble("transportCarPetrol", 0.21);
                }
                break;
            ...
            case "BIKE":
                factor = Math.max(0.0, configuredTransportFactor * bikeMultiplier);
                break;
    ...
    public double calculateFood(SurveyRequest request) {
        ...
        switch (request.getDietType().toUpperCase()) {
            ...
            case "VEGAN": {
                double veganMultiplier = settingsStoreService.getDouble("foodVeganMultiplier", 0.8);
                base = Math.max(0.0, vegFactor * veganMultiplier);
                break;
            }
    ...
    public double calculateEnergy(SurveyRequest request) {
        ...
        if (Boolean.TRUE.equals(request.getRenewable())) {
            double renewableMultiplier = settingsStoreService.getDouble("renewableEnergyMultiplier", 0.6);
            emission *= renewableMultiplier;
        }
