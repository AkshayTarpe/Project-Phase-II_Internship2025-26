...
const TABS = [
  ...
  { key: "contact-inbox", icon: "✉️", label: "Contact inbox" },
...
const ADMIN_REPORT_DATE_RANGES = [
...
const ADMIN_REPORT_CATALOG = [
  {
    id: "users",
    icon: "👥",
    category: "Users",
    title: "User Directory",
    description: "All registered non-admin users with account status and join date.",
  },
  {
    id: "emissions",
    icon: "🌍",
    ...
    description: "Carbon log history with transport, food, energy, and total emissions per entry.",
  },
  {
    id: "goals",
    icon: "🎯",
    ...
    description: "User sustainability goals with category, timeframe, progress, and status.",
  },
  {
    id: "transactions",
    icon: "💳",
    ...
    description: "Marketplace purchases with buyer, product, amount, and payment status.",
  },
  {
    id: "revenue",
    icon: "💰",
    ...
    description: "Aggregated sales and order counts grouped by marketplace product.",
  },
  {
    id: "marketplace",
    icon: "🛒",
    ...
    description: "Current marketplace listings with type, price, and carbon offset values.",
  },
  {
    id: "badges",
    icon: "🏅",
    category: "Engagement",
    title: "Badge Awards",
    description: "Badge templates and how many users have earned each badge.",
  },
  {
    id: "leaderboard",
    icon: "🏆",
    ...
    description: "Live user rankings by score, offsets purchased, goals, and badges.",
  },
  {
    id: "notifications",
    icon: "🔔",
    ...
    title: "Notifications",
    description: "System and user notifications sent through the platform.",
  },
  {
    id: "contact",
    icon: "✉️",
  ...
  },
  {
    ...
    description: "Audit trail of admin sign-ins, settings changes, and management actions.",
  },
];

const REPORT_CATEGORY_ACCENT = {
  Users: "users",
...
function getReportCategoryAccent(category) {
  return REPORT_CATEGORY_ACCENT[category] || "default";
}

/** Lifestyle survey emission factors — defaults match LIFESTYLE_SURVEY_CALCULATION.md */
const ADMIN_SURVEY_FACTOR_FIELDS = [
  {
    section: "Transport",
    accent: "transport",
    fields: [
      { key: "transportFactor", label: "Public transport", defaultValue: "0.12" },
      { key: "transportBikeMultiplier", label: "Bike", defaultValue: "0.2" },
      { key: "transportCarPetrol", label: "Car (petrol)", defaultValue: "0.21" },
      { key: "transportCarDiesel", label: "Car (diesel)", defaultValue: "0.18" },
      { key: "transportCarElectric", label: "Car (electric)", defaultValue: "0.05" },
    ],
  },
  {
    section: "Food",
    accent: "food",
    fields: [
      { key: "foodVegFactor", label: "Vegetarian", defaultValue: "1.5" },
      { key: "foodNonVegFactor", label: "Non-vegetarian", defaultValue: "3.3" },
  ...
  },
  {
    ...
    accent: "energy",
    fields: [
      { key: "electricityFactor", label: "Electricity", defaultValue: "0.82" },
      { key: "renewableEnergyMultiplier", label: "Renewable energy", defaultValue: "0.6" },
    ],
  },
];

const ADMIN_SURVEY_FACTOR_KEYS = ADMIN_SURVEY_FACTOR_FIELDS.flatMap((s) =>
...
function downloadAdminCsv(filename, headers, rows) {
  const lines = [
    headers.map(escapeAdminCsvCell).join(","),
    ...rows.map((row) => row.map(escapeAdminCsvCell).join(",")),
  ];
...
function formatAdminReportDate(iso) {
  if (!iso) return "";
  try {
...
function adminReportDateInRange(isoDate, rangeId) {
...
function buildAdminReportPayload(reportId, dateRange, data) {
  const {
    allUsers,
    adminCarbonLogs,
    adminGoals,
  ...
  const meta = ADMIN_REPORT_CATALOG.find((r) => r.id === reportId) || ADMIN_REPORT_CATALOG[0];
  const stamp = new Date().toISOString().slice(0, 10);

  if (reportId === "users") {
    const rows = getAdminFilteredUsers(allUsers, "all", "").map((u) => [
      u.id,
      u.name,
      u.email,
      u.role,
      u.status,
      formatAdminReportDate(u.createdAt),
    ]);
    return {
      ...meta,
      headers: ["User ID", "Name", "Email", "Role", "Status", "Joined"],
      rows,
      filename: `personal-carbon-tracker-users-${stamp}.csv`,
    };
  }

  if (reportId === "emissions") {
    const source = (Array.isArray(adminCarbonLogs) ? adminCarbonLogs : []).filter((l) => {
      const role = (l.user?.role || "").toString().toLowerCase();
      return !role.includes("admin");
    });
    const rows = source
      .filter((l) =>
        adminReportDateInRange(l.date || l.logDate || l.createdAt, dateRange)
      )
      .map((l) => [
        l.id,
        l.user?.name || l.user?.email || l.userName || "—",
        l.date || l.logDate || "",
        Number(l.transportEmission || 0).toFixed(2),
        Number(l.foodEmission || 0).toFixed(2),
        Number(l.energyEmission || 0).toFixed(2),
        Number(l.totalEmission || 0).toFixed(2),
      ]);
    return {
      ...meta,
      headers: ["Log ID", "User", "Date", "Transport (kg)", "Food (kg)", "Energy (kg)", "Total (kg)"],
      rows,
      filename: `personal-carbon-tracker-emissions-${stamp}.csv`,
    };
  }

  if (reportId === "goals") {
    const rows = (Array.isArray(adminGoals) ? adminGoals : [])
      .filter((g) => adminReportDateInRange(g.createdAt, dateRange))
      .map((g) => [
        g.id,
        g.userName || g.user?.name || g.userId || "—",
        g.goalTitle || g.title || g.name || "—",
        formatAdminGoalCategory(g.category),
        formatAdminGoalTimeframe(g.timeframe),
        adminGoalPeriodLabel(g.startDate, g.endDate),
        formatAdminGoalProgressLabel(parseAdminGoalProgress(g.progressPercentage, g.status === "COMPLETED")),
        g.status || "ACTIVE",
        formatAdminReportDate(g.createdAt),
      ]);
    return {
      ...meta,
      headers: ["Goal ID", "User", "Goal", "Category", "Timeframe", "Period", "Progress %", "Status", "Created"],
      rows,
      filename: `personal-carbon-tracker-goals-${stamp}.csv`,
    };
  }

  if (reportId === "transactions") {
    const rows = mapAdminTransactionRows(adminTransactions)
      .filter((t) => adminReportDateInRange(t.date, dateRange))
      .map((t) => [
        t.id,
        t.user,
        t.item,
        ...
        t.status,
        formatAdminReportDate(t.date),
      ]);
    return {
      ...meta,
      headers: ["Transaction ID", "User", "Product", "Amount (INR)", "Status", "Date"],
      rows,
      filename: `personal-carbon-tracker-transactions-${stamp}.csv`,
    };
  }

  if (reportId === "revenue") {
    const map = {};
    (Array.isArray(adminTransactions) ? adminTransactions : []).forEach((t) => {
      if (!adminReportDateInRange(t.createdAt || t.date, dateRange)) return;
      const name =
        (t.itemName || "").trim() ||
      ...
      map[name].orders += 1;
      map[name].revenue += Number(t.amount || 0);
    });
    ...
    return {
      ...meta,
      headers: ["Product", "Orders", "Revenue (INR)"],
      rows,
      filename: `personal-carbon-tracker-revenue-summary-${stamp}.csv`,
    };
  }

  if (reportId === "marketplace") {
    const rows = (Array.isArray(adminMarketplaceItems) ? adminMarketplaceItems : []).map((m) => [
      m.id,
      m.itemName || m.name || "—",
      normalizeMarketplaceCategory(m.itemType || m.type),
      Number(m.price || 0).toFixed(2),
      m.carbonOffsetValue != null ? Number(m.carbonOffsetValue).toFixed(2) : "",
      m.priceUnit || "unit",
      formatAdminReportDate(m.updatedAt || m.createdAt),
    ]);
    return {
      ...meta,
      headers: ["Item ID", "Name", "Category", "Price (INR)", "Carbon offset", "Unit", "Last updated"],
      rows,
      filename: `personal-carbon-tracker-marketplace-catalog-${stamp}.csv`,
    };
  }

  if (reportId === "badges") {
    const source = badgeTemplates.length > 0 ? badgeTemplates : adminBadges;
    const rows = (Array.isArray(source) ? source : []).map((b, idx) => [
      b.id ?? idx + 1,
      b.name || b.badgeName || "—",
      b.conditionText || b.condition || "—",
      b.active === false ? "Disabled" : "Active",
      Number(b.usersEarned ?? 0),
    ]);
    return {
      ...meta,
      headers: ["Badge ID", "Name", "Condition", "Status", "Users earned"],
      rows,
      filename: `personal-carbon-tracker-badges-${stamp}.csv`,
    };
  }

  if (reportId === "leaderboard") {
    const source = Array.isArray(adminLeaderboard) && adminLeaderboard.length
      ? adminLeaderboard
      : MOCK_LEADERBOARD;
    const rows = source.map((e) => [
      e.rank,
      e.user,
      Number(e.totalCarbonOffsetPurchased || 0).toFixed(2),
      e.goalsCompleted ?? 0,
      e.badgesEarned ?? 0,
    ...
    return {
      ...meta,
      headers: ["Rank", "User", "Carbon offset purchased", "Goals completed", "Badges earned", "Score"],
      rows,
      filename: `personal-carbon-tracker-leaderboard-${stamp}.csv`,
    };
  }

  if (reportId === "notifications") {
    const rows = normalizeAdminNotificationRows(adminNotifications)
      .filter((n) => adminReportDateInRange(n.createdAt, dateRange))
      .map((n) => [
        n.id,
        ADMIN_NOTIF_TYPE_META[n.typeKey]?.label || n.typeKey,
        n.title,
        n.message,
        n.read ? "Read" : "Unread",
        formatAdminReportDate(n.createdAt),
      ]);
    return {
      ...meta,
      headers: ["ID", "Type", "Title", "Message", "Read status", "Created"],
      rows,
      filename: `personal-carbon-tracker-notifications-${stamp}.csv`,
    };
  }

  if (reportId === "contact") {
    const rows = (Array.isArray(adminContactMessages) ? adminContactMessages : [])
      .filter((m) => adminReportDateInRange(m.createdAt, dateRange))
      .map((m) => [
        m.id,
        m.name || "—",
        m.email || "—",
        m.subject || "—",
        (m.message || "").replace(/\s+/g, " ").trim(),
        m.read === true || m.messageRead === true ? "Read" : "Unread",
        formatAdminReportDate(m.createdAt),
      ]);
    return {
      ...meta,
      headers: ["ID", "Name", "Email", "Subject", "Message", "Status", "Received"],
      rows,
      filename: `personal-carbon-tracker-contact-inbox-${stamp}.csv`,
    };
  }

  if (reportId === "audit") {
    const rows = (Array.isArray(auditLogs) ? auditLogs : [])
      .filter((l) => adminReportDateInRange(l.createdAt, dateRange))
      .map((l) => [
        l.id,
        formatAdminReportDate(l.createdAt),
        l.adminName || "—",
        l.action || "—",
        l.details || "",
        l.ipAddress || "N/A",
    ...
    return {
      ...meta,
      headers: ["ID", "Time", "Admin", "Action", "Details", "IP address"],
      rows,
      filename: `personal-carbon-tracker-admin-audit-${stamp}.csv`,
    };
  }

  return { ...meta, headers: [], rows: [], filename: `personal-carbon-tracker-report-${stamp}.csv` };
}
...
function AdminDashboard() {
  ...
  const [adminSettings, setAdminSettings] = useState({
    appName: "Personal Carbon Tracker",
    logoDataUrl: "",
    emissionThreshold: "",
    electricityFactor: "0.82",
    renewableEnergyMultiplier: "0.6",
    transportFactor: "0.12",
    transportBikeMultiplier: "0.2",
    transportCarPetrol: "0.21",
    transportCarDiesel: "0.18",
    transportCarElectric: "0.05",
    foodVegFactor: "1.5",
    foodNonVegFactor: "3.3",
    foodDairyFactor: "2.1",
    foodVeganMultiplier: "0.8",
    appVersion: "",
  ...
  const [selectedReportId, setSelectedReportId] = useState("users");
  ...
  useEffect(() => {
    if (user && (activeTab === "admin-logs" || activeTab === "reports")) {
      fetchAuditLogs();
    }
  }, [user, activeTab, fetchAuditLogs]);

  const adminReportPayload = useMemo(
    () =>
      buildAdminReportPayload(selectedReportId, reportDateRange, {
        allUsers,
        adminCarbonLogs,
        adminGoals,
        adminTransactions,
    ...
    axios
      ...
      .then((res) => {
        ...
        Promise.allSettled([
          ...
          axios.get(`${API_BASE}/api/contact/messages/admin`, { headers }),
          // Admin settings
          ...
          if (settingsResult.status === "fulfilled") {
            ...
            const factorDefaults = Object.fromEntries(
              ADMIN_SURVEY_FACTOR_FIELDS.flatMap((s) =>
                s.fields.map((f) => [f.key, f.defaultValue])
              )
            );
            setAdminSettings({
              appName: settings.appName ?? "Personal Carbon Tracker",
              logoDataUrl: settings.logoDataUrl ?? "",
              emissionThreshold: settings.emissionThreshold ?? "",
              ...factorDefaults,
              ...Object.fromEntries(
                ADMIN_SURVEY_FACTOR_KEYS.map((key) => [
                  key,
  ...
  function renderReports() {
    const reportCategories = [
      "all",
      ...new Set(ADMIN_REPORT_CATALOG.map((r) => r.category)),
    ];
    const catalogFiltered =
      reportCategoryFilter === "all"
        ? ADMIN_REPORT_CATALOG
        : ADMIN_REPORT_CATALOG.filter((r) => r.category === reportCategoryFilter);

    const { headers, rows, filename, title, description } = adminReportPayload;
    const previewRows = rows.slice(0, 15);
    const totalRevenue = rows.reduce((sum, row) => {
      if (selectedReportId !== "transactions") return sum;
      return sum + Number(row[3] || 0);
    }, 0);

    const handleExport = () => {
      if (!rows.length) {
        setReportsMessage("No data to export for the selected report and date range.");
        return;
      }
      downloadAdminCsv(filename, headers, rows);
      setReportsMessage(`Exported ${rows.length} row${rows.length === 1 ? "" : "s"} to ${filename}.`);
    };

    const selectedMeta =
      ADMIN_REPORT_CATALOG.find((r) => r.id === selectedReportId) || ADMIN_REPORT_CATALOG[0];
    const totalRevenueAll = mapAdminTransactionRows(adminTransactions).reduce(
      (s, t) => s + t.amount,
      0
    );
    const userCount = getAdminFilteredUsers(allUsers, "all", "").length;
    const dateRangeLabel =
      ADMIN_REPORT_DATE_RANGES.find((r) => r.id === reportDateRange)?.label || "All time";
    const showDateFilter =
      selectedReportId !== "users" &&
      selectedReportId !== "marketplace" &&
      selectedReportId !== "badges" &&
      selectedReportId !== "leaderboard";

    return (
      <div className="admin-reports-page">
        <div className="admin-reports-topbar">
          <div className="admin-reports-topbar-stats">
            <span className="admin-reports-topbar-stat">
              <strong>{userCount}</strong> users
            </span>
            <span className="admin-reports-topbar-dot" aria-hidden />
            <span className="admin-reports-topbar-stat">
              <strong>₹{totalRevenueAll.toFixed(0)}</strong> revenue
            </span>
            <span className="admin-reports-topbar-dot" aria-hidden />
            <span className="admin-reports-topbar-stat">
              <strong>{rows.length}</strong> rows in view
            </span>
          </div>
        </div>

        <section className="admin-reports-picker" aria-label="Choose a report">
          <div className="admin-reports-picker-head">
            <h3 className="admin-reports-picker-title">Choose a report to run</h3>
            <p className="admin-reports-picker-sub">
              Pick a card below — each opens a live preview and CSV export. This is your reports hub, not a copy of the admin menu.
            </p>
          </div>
          <div className="admin-reports-chips" role="tablist" aria-label="Report categories">
            {reportCategories.map((cat) => (
              <button
                key={cat}
                type="button"
                role="tab"
                aria-selected={reportCategoryFilter === cat}
                className={`admin-reports-chip ${reportCategoryFilter === cat ? "active" : ""}`}
          ...
          <div className="admin-reports-card-grid">
            {catalogFiltered.map((report) => (
              <button
                key={report.id}
                type="button"
                className={`admin-reports-type-card ${
                  selectedReportId === report.id ? "active" : ""
                }`}
                data-accent={getReportCategoryAccent(report.category)}
                onClick={() => {
                  setSelectedReportId(report.id);
                  setReportsMessage("");
                }}
              >
                <span className="admin-reports-type-card-icon" aria-hidden>
                  {report.icon}
                </span>
                <span className="admin-reports-type-card-category">{report.category}</span>
                <span className="admin-reports-type-card-title">{report.title}</span>
                <span className="admin-reports-type-card-desc">{report.description}</span>
              </button>
            ))}
          </div>
        </section>

        <section className="admin-reports-workspace" aria-label="Report preview">
          <div className="admin-reports-workspace-label">Preview &amp; export</div>
          <div
            className="admin-reports-workspace-banner"
            data-accent={getReportCategoryAccent(selectedMeta.category)}
          >
            <div className="admin-reports-workspace-banner-main">
              <span className="admin-reports-workspace-icon" aria-hidden>
                {selectedMeta.icon}
              </span>
              <div>
                <h3 className="admin-reports-workspace-title">{title}</h3>
                <p className="admin-reports-workspace-desc">{description}</p>
              </div>
            </div>
            <div className="admin-reports-workspace-badges">
              <span className="admin-reports-badge">{selectedMeta.category}</span>
              <span className="admin-reports-badge admin-reports-badge--muted">
          ...
          <div className="admin-reports-toolbar-bar">
            {showDateFilter && (
              <label className="admin-reports-date-field">
                <span className="admin-reports-date-label">Period</span>
                <select
                ...
                >
                  {ADMIN_REPORT_DATE_RANGES.map((r) => (
            ...
            {selectedReportId === "audit" && (
              <button
                type="button"
                className="admin-reports-btn admin-reports-btn--ghost"
                onClick={() => {
                  setReportsMessage("");
                  fetchAuditLogs();
                }}
            ...
            <button
              type="button"
              className="admin-reports-btn admin-reports-btn--export"
              onClick={handleExport}
              disabled={!rows.length}
            >
              <span className="admin-reports-btn-icon" aria-hidden>
                ⬇
              </span>
              Export CSV
            </button>
          ...
          {reportsMessage && (
            <p
              className={`admin-reports-toast ${
                reportsMessage.toLowerCase().includes("failed") ||
                reportsMessage.toLowerCase().includes("no data")
                  ? "is-error"
                  : "is-success"
              }`}
            >
          ...
          {selectedReportId === "audit" && auditLogsError && (
            <p className="admin-reports-toast is-error">{auditLogsError}</p>
          )}

          {selectedReportId === "transactions" && rows.length > 0 && (
            <p className="admin-reports-revenue-callout">
              Filtered revenue: <strong>₹{totalRevenue.toFixed(2)}</strong>
            </p>
          )}

          <div className="admin-reports-table-panel">
            {rows.length > 0 ? (
              <>
                <div className="admin-reports-table-scroll">
                  <table className="admin-reports-data-table">
                    <thead>
                      <tr>
                        {headers.map((h) => (
                          <th key={h} scope="col">
                      ...
                      {previewRows.map((row, idx) => (
                        <tr key={`report-row-${idx}`}>
                          {row.map((cell, cellIdx) => (
                            <td key={`${idx}-${cellIdx}`} title={String(cell)}>
                              {cell}
                            </td>
                          ))}
                        </tr>
                ...
                {rows.length > previewRows.length && (
                  <footer className="admin-reports-table-footer">
                    Previewing {previewRows.length} of {rows.length} rows — export CSV for the full dataset.
                  </footer>
                )}
              </>
            ) : (
              <div className="admin-reports-empty">
                <span className="admin-reports-empty-icon" aria-hidden>
                  📊
                </span>
                <p className="admin-reports-empty-title">No data for this selection</p>
                <p className="admin-reports-empty-hint">
                  Widen the date range or wait until users generate activity in this area.
                </p>
              </div>
            )}
          </div>
  ...
  function renderContactInbox() {
    ...
    function openContactReplyModal(m) {
      ...
      setContactReplySubject(orig ? `Re: ${orig}` : "Re: Your Personal Carbon Tracker contact message");
    ...
    async function submitContactReply() {
      ...
      try {
        const res = await axios.post(
          `${API_BASE}/api/contact/messages/${contactReplyModal.id}/reply`,
    ...
    async function markContactRowRead(id) {
      ...
      try {
        const res = await axios.patch(
          `${API_BASE}/api/contact/messages/${id}/read`,
          {},
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const updated = res.data;
  ...
  function renderSettings() {
    const buildSurveyFactorPayload = () => {
      const payload = {
        maintenanceMode: Boolean(adminSettings.maintenanceMode),
      };
      for (const key of ADMIN_SURVEY_FACTOR_KEYS) {
        const raw = adminSettings[key];
        const n = Number(raw);
        if (raw === "" || raw == null || Number.isNaN(n) || n < 0) {
          return { error: `Invalid value for ${key}. Enter a number ≥ 0.` };
        }
        payload[key] = n;
      }
      return { payload };
    ...
    return (
      <div className="admin-table-card admin-settings-page-card">
        {settingsMessage && (
        ...
        <form
          onSubmit={async (e) => {
            ...
            const built = buildSurveyFactorPayload();
            if (built.error) {
              setSettingsMessage(built.error);
              return;
            }
            ...
            try {
              const headers = { Authorization: `Bearer ${token}` };
              await axios.put(`${API_BASE}/api/admin/settings`, built.payload, { headers });
        ...
        >
          <section className="admin-survey-factors-panel">
            <header className="admin-survey-factors-head">
              <div>
                <h3 className="admin-survey-factors-title">Lifestyle survey factors</h3>
                <p className="admin-survey-factors-sub">Values used when users submit the lifestyle survey.</p>
              </div>
              {(adminSettings.lastUpdatedAt || adminSettings.lastUpdatedBy) && (
                <p className="admin-survey-factors-meta">
                  Last saved
                  {adminSettings.lastUpdatedBy ? ` · ${adminSettings.lastUpdatedBy}` : ""}
                  {adminSettings.lastUpdatedAt
                    ? ` · ${new Date(adminSettings.lastUpdatedAt).toLocaleString("en-IN", {
                        dateStyle: "medium",
                        timeStyle: "short",
                      })}`
                    : ""}
            ...
            <div className="admin-survey-factors-grid">
              {ADMIN_SURVEY_FACTOR_FIELDS.map((section) => (
                <div
                  key={section.section}
                  className="admin-survey-factor-group"
                  data-accent={section.accent}
                >
                  <h4 className="admin-survey-factor-group-title">{section.section}</h4>
                  <ul className="admin-survey-factor-list">
                    {section.fields.map((field) => (
                      <li key={field.key} className="admin-survey-factor-row">
                        <label className="admin-survey-factor-label" htmlFor={`setting-${field.key}`}>
                          {field.label}
                        </label>
                        <input
                          ...
                          step="0.01"
                          inputMode="decimal"
                          className="admin-survey-factor-input"
                          value={adminSettings[field.key] ?? ""}
                          onChange={(e) =>
          ...
          <section className="admin-settings-maintenance-panel">
            <h4 className="admin-survey-factor-group-title">Maintenance</h4>
            <label className="admin-settings-switch-row admin-settings-switch-row--inline">
              <input
                type="checkbox"
              ...
              <span className={`admin-settings-switch-text ${adminSettings.maintenanceMode ? "is-on" : "is-off"}`}>
                {adminSettings.maintenanceMode ? "Maintenance ON" : "Site live"}
              </span>
            </label>
          </section>

          <div className="admin-settings-actions admin-settings-actions--footer">
            <button
              type="button"
              className="admin-action-btn"
              ...
              onClick={() => {
                ...
                setAdminSettings((prev) => ({ ...prev, ...defaults }));
                setSettingsMessage("Reset to defaults — click Save to apply.");
              }}
            >
              Reset defaults
            </button>
  ...
  return (
    <AppLayout>
      <div className="admin-page">
        ...
        <div className={`admin-content${activeTab === "reports" ? " admin-content--reports" : ""}`}>
          <div className="admin-content-header">
            ...
            <p className="admin-content-subtitle">
              ...
              {activeTab === "reports" &&
                "Generate, preview, and export platform reports (users, emissions, sales, goals, and more)."}
              {activeTab === "admin-logs" && "View consolidated activity logs for admin monitoring."}
              {activeTab === "settings" &&
                "Edit lifestyle survey emission factors and maintenance mode."}
            </p>
          </div>
          {renderContent()}
        </div>
